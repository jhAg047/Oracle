...
-- EMPLOYEE테이블에서 사원이 추가되면 '신입사원이 입사했습니다'라는 문구 출력되는 TRG_01 트리거 생성
...
-- 상품 입출고 테이블에 입고나 출고가 들어갈 때마다 PRODECT에 있는 STOCK이 바뀌어야함
-- PRO_DETAIL 테이블에 INSERT 후에 STATUS컬럼 값에 따른 PRODUCT 테이블 변경
CREATE OR REPLACE TRIGGER TRG_02
AFTER INSERT ON PRO_DETAIL
FOR EACH ROW 
BEGIN
    IF :NEW.STATUS = '입고' -- PRO_DETAIL에 STATUS 컬럼 값으로 새로 들어간 값이 '입고' 면
    THEN 
        UPDATE PRODUCT

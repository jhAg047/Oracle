-- SELECT
-- Result Set : SELET 구문으로 데이터를 조회한 결과물. 반환된 행들의 집합(0행 이상)
...
-- 컬럼 값 산술 연산
-- SELECT 시 컬럼 명 입력 부분에 계산에 필요한 컬럼 명, 숫자, 연산자를 이용해서 결과를 조회 가능
...
SELECT *, SALARY*12     -- 아스트로는 혼자 써야 됨
FROM EMPLOYEE;
...
-- SYSDATE 오늘 날짜 출력
-- DUAL 가상테이블
SELECT EMP_NAME, HIRE_DATE, ROUND(SYSDATE - HIRE_DATE) -- FLOOR는 내림, ROUND는 반올림, CEIL는 올림
FROM EMPLOYEE;
...
SELECT EMP_NAME AS 이름, SALARY*12 AS "연봉(원)", SALARY*(1+BONUS)*12 AS "총소득(원)"
FROM EMPLOYEE;

-- EMPLOYEE테이블에서 이름, 고용일, 근무일수(오늘날짜 - 고용일) 조회
SELECT EMP_NAME AS 이름 , HIRE_DATE AS 고용일 , SYSDATE-HIRE_DATE AS 근무일수
FROM EMPLOYEE;

-- 0204

-- 리터럴 : 임의로 정한 문자열을 SELECT절에 사용하면 테이블에 존재하는 데이터처럼 사용가능
-- 문자나 날짜 리터럴은 ' '기호 사용되며 모든 행에 반복 표시 됨
-- 테이블 안에는 없는데 있는것처럼 지정해서 뽑아내는거

-- EMPLOYEE 테이블에서 직원의 직원 번호, 사원 명, 급여, 단위(데이터 값: 원) 조회
SELECT EMP_ID "직원 번호" ,EMP_NAME "사원 명",SALARY 급여, '원' AS 단위
FROM EMPLOYEE;

-- DISTINCT : 컬럼에 포함된 중복값을 한 번씩만 표기하고자 할 때 사용, SELECT절에 딱 한 번만 쓸 수 있음
...
-- EMMPLOYEE테이블에서 부서코드와 직급 코드를 중복제거 하여 조회
-- 2개가 있으면 둘이 묶여서 검색되는거임~!
...
SELECT EMP_ID 사번, EMP_NAME 이름, HIRE_DATE,'재직중' AS 근무여부 
FROM EMPLOYEE
WHERE ENT_YN ='N';
...
-- 논리 연산자 : AND / OR
-- EMPLOYEE 부서코드가 'D6'이고 급여를 2백만원보다 많이 받는 직원의 이름, 부서코드, 급여 조회
...
-- 미니 실습 문제
-- 1. EMPLOYEE테이블에 월급이 4000000이상이고 JOB_CODE가 J2인 사원의 전체 내용 조회
SELECT *
FROM EMPLOYEE
WHERE SALARY >= 4000000 AND JOB_CODE = 'J2';
-- 2. EMPLOYEE테이블에 DEPT_CODE가 D9이거나 D5인 사원 중 고용일이 02년 1월 1일 보다 빠른 사원의 이름, 부서코드, 고용일 조회
...
-- 급여를 3500000보다 많이 받고 6000000 보다 적게 받는 사원의 이름, 급여 조회
-- 1) BETWEEN AND 미 적용
SELECT EMP_NAME,SALARY
FROM EMPLOYEE
WHERE SALARY >= 3500000 AND SALARY <= 6000000;
...
-- 반대로 급여를 350만원 미만, 또는 600만원을 초과하는 직원의 사번, 이름, 급여, 부서코드, 직급 코드
-- NOT 컬럼명 BETWEEN AND
-- 컬럼명 NOT BETWEEN AND
SELECT EMP_ID,EMP_NAME,SALARY, DEPT_CODE,JOB_CODE
FROM EMPLOYEE
...
-- 이메일 중 앞글자가 3자리인 이메일주소를 가진 사원의 사번, 이름,이메일 주소 조회
--SELECT EMP_ID,EMP_NAME,EMAIL
--FROM EMPLOYEE
-- WHERE EMAIL LIKE '____%'; - 원하는 결과값이 나오지 않음
...
SELECT EMP_ID,EMP_NAME,EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '___1_%' ESCAPE '1';

-- NOT LIKE : 특정 패턴을 만족시키지 않은 값 조회 
...
-- EMPLOYEE테이블에서 관리자도 없고 부서배치도 받지 않은 직원의 이름, 관리자 부서 코드 조회
...
-- EMPLOYEE테이블에서 부서코드가 D6이거나 D9인 사원의 이름, 부서코드, 급여 조회
SELECT EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
...
SELECT EMP_NAME||'의 월급은 ' || SALARY || '원입니다.' AS 월급형식
FROM EMPLOYEE;
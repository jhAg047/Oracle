...
--ORDER BY DEPT_CODE NULLS FIRST; -- NULL을 제일 먼저두고 정렬
--ORDER BY 2; -- 2번째 컬럼(이름)으로 오름차순 / 근데 안쓰는게 좋음
ORDER BY 급여; -- 별칭으로도 정렬할수있음

/*  SELECT문 전체
    5 - SELECT : 조회
    1 - FROM : 참조할 테이블명(판)/ 내가 SELECT한 결과가 테이블이 될수도 있음
    2 - WHERE : 조건 
    3 - GROUP BY 
    4 - HAVING
    6 - ORDER BY : 정렬
...
SELECT DEPT_CODE,SUM(SALARY) -- 결과값의 개수가 달라서 오류
FROM EMPLOYEE;
...
SELECT DEPT_CODE,SUM(SALARY) 급여합계 ,ROUND(AVG(SALARY)) 급여평균,COUNT(*) 인원수
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE;

-- EMPLOYEE테이블에서 직급코드, 보너스를 받는 사원수를 조회하여 직급코드 순으로 오름차순 정렬
SELECT JOB_CODE, COUNT(BONUS) "보너스받는 사원수"
FROM EMPLOYEE
...
-- 부서코드와 급여 300만원 이상인 직원의 그룹별 평균 급여 조회
...
-- ROLLUP함수 : 그룹 별로 중간 집계 처리 후 총 합계
...
-- GROUPING 함수 
-- ROLLUP이나 CUBE에 의한 산출물이 인자로 전달받은 컬럼의 산출물이면 0을 반환, 아니면 1 반환
SELECT  DEPT_CODE,JOB_CODE,SUM(SALARY),
...
--SELECT EMP_NAME,DEPT_CODE,EMP_NO,TO_DATE(SUBSTR(EMP_NO,1,6),'YYMMDD')
--FROM EMPLOYEE
--WHERE NOT SUBSTR(EMP_NO,3,4) > 1231;
...
-- DEPT_CODE가 D5이거나 급여가 300만원을 초과하는 직원의 사번, 부서코드, 급여 조회
